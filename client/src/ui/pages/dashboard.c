...
static int selected_option = 0; // 0: Create Room, 1: Join Room, 2: Logout
...
void page_dashboard_draw(ClientContext* ctx) {
    ...
    mvprintw(menu_start_y, (col - 20)/2, "[ Create Room ]");
    if (selected_option == 0) attroff(A_REVERSE);

    if (selected_option == 1) attron(A_REVERSE);
    mvprintw(menu_start_y + 2, (col - 20)/2, "[ Join Room ]");
