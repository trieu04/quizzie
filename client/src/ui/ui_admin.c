...
void ui_show_admin_dashboard(GtkWidget** window,
  ...
  *window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
  ...
  GtkWidget* tree_view =
      gtk_tree_view_new_with_model(GTK_TREE_MODEL(room_store));

  GtkCellRenderer* renderer = gtk_cell_renderer_text_new();
  gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(tree_view), -1,
                                              "ID", renderer, "text", 0, NULL);
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(tree_view), -1, "Name", renderer, "text", 1, NULL);
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(tree_view), -1, "Status", renderer, "text", 2, NULL);
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(tree_view), -1, "Open Time", renderer, "text", 3, NULL);
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(tree_view), -1, "Close Time", renderer, "text", 4, NULL);
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(tree_view), -1, "Questions", renderer, "text", 5, NULL);
  gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(tree_view), -1,
                                              "Allowed Attempts", renderer,
                                              "text", 6, NULL);
  ...
  g_signal_connect(create_room_btn, "clicked",
                   G_CALLBACK(on_create_room_clicked), *window);
  gtk_box_pack_start(GTK_BOX(action_box), create_room_btn, FALSE, FALSE, 10);

  GtkWidget* manage_questions_btn =
      gtk_button_new_with_label("Manage Question Banks");
  g_signal_connect(manage_questions_btn, "clicked",
                   G_CALLBACK(on_manage_questions_clicked), *window);
  gtk_box_pack_start(GTK_BOX(action_box), manage_questions_btn, FALSE, FALSE,
                     10);
  ...
  g_signal_connect(details_btn, "clicked", G_CALLBACK(on_room_details_clicked),
                   tree_view);
...
static void show_create_room_dialog(GtkWidget* parent) {
  ...
  g_signal_connect(preview_btn, "clicked", G_CALLBACK(on_preview_bank_clicked),
                   bank_combo);
  ...
  if (receive_packet(ui_get_socket(), type, &resp) == 0 &&
    ...
    cJSON_ArrayForEach(item, data) {
      gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(bank_combo),
                                     item->valuestring);
    }
    cJSON_Delete(resp);
  }
  ...
  gtk_grid_attach(GTK_GRID(grid),
                  gtk_label_new("Open Time (YYYY-MM-DD HH:MM:SS):"), 0, 2, 1,
                  1);
  ...
  gtk_grid_attach(GTK_GRID(grid),
                  gtk_label_new("Close Time (YYYY-MM-DD HH:MM:SS):"), 0, 3, 1,
                  1);
  ...
  gtk_grid_attach(GTK_GRID(grid), gtk_label_new("Allowed Attempts:"), 0, 5, 1,
                  1);
  ...
  if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
    ...
    char* bank =
        gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(bank_combo));
...
static void on_editor_save(GtkWidget* btn, gpointer data) {
  ...
  GtkTreeIter iter;
  gboolean valid =
      gtk_tree_model_get_iter_first(GTK_TREE_MODEL(ctx->store), &iter);
  while (valid) {
    char *q, *a, *b, *c, *d;
    int correct;
    gtk_tree_model_get(GTK_TREE_MODEL(ctx->store), &iter, 0, &q, 1, &a, 2, &b,
                       3, &c, 4, &d, 5, &correct, -1);
...
static void on_editor_add(GtkWidget* btn, gpointer data) {
  (void)btn;
  EditorCtx* ctx = (EditorCtx*)data;
  gtk_list_store_insert_with_values(ctx->store, NULL, -1, 0, "New Question", 1,
                                    "Op1", 2, "Op2", 3, "Op3", 4, "Op4", 5, 0,
                                    -1);
}
...
static void on_cell_edited(GtkCellRendererText* renderer,
  ...
  EditorCtx* ctx = (EditorCtx*)data;
  GtkTreeIter iter;
  if (gtk_tree_model_get_iter_from_string(GTK_TREE_MODEL(ctx->store), &iter,
                                          path_string)) {
...
static void show_question_bank_editor(GtkWidget* parent, const char* bank_id) {
  GtkWidget* dialog = gtk_dialog_new_with_buttons(
      "Question Bank Editor", GTK_WINDOW(parent),
  ...
  if (receive_packet(ui_get_socket(), type, &resp) == 0 &&
    ...
    cJSON_ArrayForEach(item, data) {
      ...
      gtk_list_store_insert_with_values(ctx->store, NULL, -1, 0, q->valuestring,
                                        1, op[0], 2, op[1], 3, op[2], 4, op[3],
                                        5, corr->valueint, -1);
    }
    cJSON_Delete(resp);
  }
  ...
  const char* headers[] = {"Question", "Op A", "Op B",
                           "Op C",     "Op D", "Correct Idx"};
  for (int i = 0; i < 6; i++) {
    ...
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(tree), -1,
                                                headers[i], r, "text", i, NULL);
  }

  gtk_box_pack_start(GTK_BOX(vbox), gtk_scrolled_window_new(NULL, NULL), TRUE,
                     TRUE, 0);
  gtk_container_add(GTK_CONTAINER(g_list_nth_data(
                        gtk_container_get_children(GTK_CONTAINER(vbox)), 0)),
                    tree);
...
static void refresh_bank_list(GtkTreeView* tree) {
  ...
  if (receive_packet(ui_get_socket(), type, &resp) == 0 &&
    ...
    cJSON* item;
    cJSON_ArrayForEach(item, data) {
      gtk_list_store_insert_with_values(store, NULL, -1, 0, item->valuestring,
                                        -1);
    }
    cJSON_Delete(resp);
  }
...
static void on_upload_csv_clicked(GtkWidget* btn, gpointer data) {
  ...
  if (!filename || strlen(bank_name) == 0) {
    GtkWidget* msg = gtk_message_dialog_new(
        GTK_WINDOW(dialog_window), GTK_DIALOG_MODAL, GTK_MESSAGE_WARNING,
  ...
  if (f) {
    ...
    GtkWidget* msg = gtk_message_dialog_new(
        GTK_WINDOW(dialog_window), GTK_DIALOG_MODAL, GTK_MESSAGE_INFO,
    ...
    GtkWidget* msg = gtk_message_dialog_new(
        GTK_WINDOW(dialog_window), GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR,
...
static void show_manage_questions_dialog(GtkWidget* parent) {
  GtkWidget* dialog = gtk_dialog_new_with_buttons(
      "Manage Question Banks", GTK_WINDOW(parent),
  ...
  GtkWidget* chooser = gtk_file_chooser_button_new(
      "Select CSV File", GTK_FILE_CHOOSER_ACTION_OPEN);
  ...
  gtk_box_pack_start(GTK_BOX(hbox_imp), gtk_label_new("Name:"), FALSE, FALSE,
                     5);
  ...
  GtkWidget* lblHint = gtk_label_new(
      "CSV Format: Question,OptionA,OptionB,OptionC,OptionD,CorrectIndex(0-3)");
  ...
  GtkCellRenderer* renderer = gtk_cell_renderer_text_new();
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(tree), -1, "Bank Name (ID)", renderer, "text", 0, NULL);
  ...
  g_signal_connect(btnEdit, "clicked", G_CALLBACK(on_manage_edit_clicked),
                   tree);
  g_signal_connect(btnDelete, "clicked", G_CALLBACK(on_manage_delete_clicked),
                   tree);
  ...
  if (receive_packet(ui_get_socket(), type, &resp) == 0 &&
    ...
    cJSON* item;
    cJSON_ArrayForEach(item, data) {
      gtk_list_store_insert_with_values(bank_store, NULL, -1, 0,
                                        item->valuestring, -1);
    }
    cJSON_Delete(resp);
  }
  ...
  g_signal_connect(btnUpload, "clicked", G_CALLBACK(on_upload_csv_clicked),
                   widgets);
...
static void on_delete_room_confirm(GtkWidget* btn, gpointer data) {
  const char* room_id =
      (const char*)g_object_get_data(G_OBJECT(btn), "room_id");
  GtkWidget* parent_dialog = (GtkWidget*)data;  // The Room Details dialog

  GtkWidget* msg =
      gtk_message_dialog_new(GTK_WINDOW(parent_dialog), GTK_DIALOG_MODAL,
...
static void show_room_details_dialog(GtkWidget* parent,
                                     const char* room_id,
                                     const char* room_name) {
  GtkWidget* dialog = gtk_dialog_new_with_buttons(
      "Room Details", GTK_WINDOW(parent),
      GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, "Close",
      GTK_RESPONSE_CANCEL, NULL);
  ...
  gtk_grid_attach(GTK_GRID(grid), gtk_label_new("Question Bank:"), 0, row, 1,
                  1);
  gtk_grid_attach(GTK_GRID(grid), lblBank, 1, row++, 1, 1);

  gtk_grid_attach(GTK_GRID(grid), gtk_label_new("Num Questions:"), 0, row, 1,
                  1);
  gtk_grid_attach(GTK_GRID(grid), lblNumQ, 1, row++, 1, 1);

  gtk_grid_attach(GTK_GRID(grid), gtk_label_new("Allowed Attempts:"), 0, row, 1,
                  1);
  ...
  col = gtk_tree_view_column_new_with_attributes("Username", renderer, "text",
                                                 0, NULL);
  gtk_tree_view_column_set_sort_column_id(col, 0);
  gtk_tree_view_append_column(GTK_TREE_VIEW(tree), col);

  col = gtk_tree_view_column_new_with_attributes("Score", renderer, "text", 1,
                                                 NULL);
  gtk_tree_view_column_set_sort_column_id(col, 1);
  gtk_tree_view_append_column(GTK_TREE_VIEW(tree), col);

  col = gtk_tree_view_column_new_with_attributes("Time", renderer, "text", 2,
                                                 NULL);
  ...
  if (receive_packet(ui_get_socket(), type, &resp) == 0 &&
    ...
    if (data) {
      ...
      snprintf(stat_str, sizeof(stat_str),
               "Total Attempts: %d   Average Score: %.2f", total, avg);
      ...
      cJSON_ArrayForEach(res, results) {
        ...
        gtk_list_store_insert_with_values(res_store, NULL, -1, 0,
                                          u ? u->valuestring : "?", 1,
  ...
  g_object_set_data_full(G_OBJECT(btnDel), "room_id", g_strdup(room_id),
                         g_free);
  g_signal_connect(btnDel, "clicked", G_CALLBACK(on_delete_room_confirm),
                   dialog);
  gtk_container_add(GTK_CONTAINER(bbox), btnDel);
  ...
  cJSON_ArrayForEach(room, rooms_array) {
    ...
    if (id && name) {
      ...
      gtk_list_store_insert_with_values(
          room_store, NULL, -1, 0, id->valuestring, 1, name->valuestring, 2,
